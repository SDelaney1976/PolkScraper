...
def export_today_merge_data_with_gui() -> Path:
    ...
    # 5) Remove duplicate addresses (keep first occurrence)
    if "Address 1" in filtered.columns:
        before_dedup = len(filtered)
        filtered = filtered.drop_duplicates(subset=["Address 1"], keep="first")
        after_dedup = len(filtered)
        duplicates_removed = before_dedup - after_dedup
        
        if duplicates_removed > 0:
            print(f"🔁 Removed {duplicates_removed} duplicate address row(s), kept first occurrence")
        
        if filtered.empty:
            messagebox.showwarning(
                "No rows after deduplication",
                "All rows were duplicates and have been removed.\n"
                "Nothing will be exported."
            )
            raise RuntimeError("Export canceled: no rows remained after duplicate removal.")
