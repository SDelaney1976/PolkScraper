# src/exports/generate_english_letters.py
import os
import sys
...
def _filter_non_hispanic(df: pd.DataFrame) -> pd.DataFrame:
    # Keep rows where Race != 'Hispanic' (case-insensitive). If Race missing, keep the row.
...
def _open_word_template_picker():
    root = tk.Tk(); root.withdraw()
    path = filedialog.askopenfilename(
        title="Open the English Word letter template (.docx/.docm)",
...
def main():
    # 1) Pick source Excel
    src = _pick_excel("Select the Excel to pull NON-Hispanic rows from")
    if not src:
    ...
    try:
        ...
        df_out = _filter_non_hispanic(df)
    except Exception as e:
        messagebox.showerror("Read/Filter Error", f"Could not process Excel:\n{e}")
        return

    if df_out.empty:
        messagebox.showinfo("No Rows", "No NON-Hispanic rows found in the selected file.")
        return

    # 4) Save to <repo>/Output/English_MergeData_<date>.xlsx
    OUTPUT_DIR = get_export_dir()
    out_path = OUTPUT_DIR / f"English_MergeData_{cap_date}.xlsx"
    ...
    messagebox.showinfo("Export complete", f"Saved:\n{out_path}\n\nNext: pick your English Word template.")
    tpl = _open_word_template_picker()
    if tpl:
        try:
            # macOS/Linux/Windows open
