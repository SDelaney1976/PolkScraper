...
# -------------------------------
# Ensure validator package is importable (esp. in frozen app)
# -------------------------------
def _ensure_validator_on_path():
    """
    In a frozen app, try common bundle locations so 'import validator' works.
    This is safe/no-op when already importable.
    """
    if "validator" in sys.modules:
        return

    candidates = []
    # 1) PyInstaller MEIPASS (temp dir with unpacked files)
    base = Path(getattr(sys, "_MEIPASS", "")) if getattr(sys, "_MEIPASS", None) else None
    if base:
        candidates += [base, base / "_internal", base / "Frameworks"]

    # 2) Dir of the executable (Windows EXE / macOS MacOS/)
    exe_dir = Path(sys.argv[0]).resolve().parent
    candidates += [exe_dir, exe_dir / "_internal", exe_dir / "Frameworks", exe_dir.parent / "Frameworks"]

    # 3) Current working dir (dev)
    candidates.append(Path.cwd())

    for root in candidates:
        try:
            if (root / "validator").exists():
...
# Try to import validator; if it fails in frozen, fix path and retry
try:
    ...
    try:
        ...
        # Leave it; run-time call will still try after --run-module path fix
        pass

# Nudge PyInstaller to bundle these submodules (import is safe; __main__ blocks prevent execution)
try:
    import validator.validate_address  # noqa: F401
except Exception:
    pass
try:
    import validator.final_scrub  # noqa: F401
except Exception:
    pass
...
# -------------------------------
# Generic module runner (validator / final cleanup)
# -------------------------------
...
def show_report_window(parent: tk.Tk, title: str, body: str, autosave: bool = True):
    ...
    ttk.Label(frm, text=hdr, font=("", 12, "bold"), justify="left").pack(anchor="w", pady=(0, 8))
...
def build_ui():
    ...
    def run_task(module_name: str, success_msg: str, show_full_output: bool = False):
        set_running(True)
        ...
        def worker():
            ...
            def done():
                ...
                if rc == 0:
                    if show_full_output:
                        body = (out or "").strip() or success_msg
                        show_report_window(root, f"{module_name} — Report ✅", body, autosave=True)
                    else:
                        messagebox.showinfo("Success", success_msg, parent=root)
                    status_lbl.config(text="Success.")
                else:
                    details = ((err or "") + ("\n" + (out or "") if out else "")).strip()
                    if show_full_output and details:
                        show_report_window(root, f"{module_name} — Error ❌", details, autosave=True)
                    else:
                        messagebox.showerror("Error", f"{module_name} exited with code {rc}.\n\n{details}", parent=root)
    ...
    ttk.Button(
        ...
        command=lambda: run_task("validator.validate_address", "Validation completed successfully.", show_full_output=False)
    ).pack(fill="x", pady=(0, 6), ipadx=6, ipady=3)

    ttk.Button(
        btns, text="Final Cleanup 🧹",
        command=lambda: run_task("validator.final_scrub", "Final cleanup completed successfully.", show_full_output=True)
    ).pack(fill="x", pady=(0, 6), ipadx=6, ipady=3)
...
def _run_module_entry(modname: str):
    # Make sure bundled package is importable inside child
    _ensure_validator_on_path()
    import runpy
    # Clean argv so child module doesn't see our launcher flags
    sys.argv = [modname]
    try:
