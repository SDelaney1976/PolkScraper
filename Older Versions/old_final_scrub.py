...
# --- Mc* last-name fixer ---
def _fix_mc_token(token: str) -> str:
    """
    If a token starts with 'Mc' in any case, force the 3rd letter uppercase.
    Handles hyphenated surnames by fixing each segment.
    """
    def fix_one(p: str) -> str:
        # case-insensitive match for 'Mc' + letter + rest
        m = re.match(r'(?i)^mc([A-Za-z])(.*)$', p)
        if not m:
            return p
    ...
    if not isinstance(token, str):
        return token
    parts = token.split('-')
    parts = [fix_one(p) for p in parts]
    return '-'.join(parts)
...
def proper_case_name(name):
    ...
    # Apply Mc* rule to the last non-suffix token (i.e., surname)
    if out:
        # Find index of last non-suffix token
        idx = None
        for i in range(len(out) - 1, -1, -1):
            if out[i] not in SUFFIXES:
                idx = i
                break
        if idx is not None:
            out[idx] = _fix_mc_token(out[idx])
